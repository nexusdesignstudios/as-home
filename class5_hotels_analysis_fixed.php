...
$columns = DB::select("DESCRIBE propertys");
$classColumn = null;
foreach ($columns as $column) {
    if (stripos($column->Field, 'class') !== false) {
        $classColumn = $column->Field;
        echo "   Found class column: {$classColumn}\n";
        break;
    }
}
if (!$classColumn) {
    echo "   No class column found, checking other relevant columns...\n";
    foreach ($columns as $column) {
        if (stripos($column->Field, 'star') !== false || stripos($column->Field, 'category') !== false) {
            echo "   Found: {$column->Field}\n";
        }
    }
}

echo "\n🏨 HOTELS 387 & 388 - BASIC INFO:\n";

// Get hotels without class filter first
...
foreach ($datesToCheck as $date) {
    ...
    $xirosesDisplay = $xirosesPeriodType ? $xirosesPeriodType : 'none';
    $ismailiaDisplay = $ismailiaPeriodType ? $ismailiaPeriodType : 'none';
    
    echo "   {$date} | {$xirosesStatus} ({$xirosesDisplay}) | {$ismailiaStatus} ({$ismailiaDisplay})\n";
}
